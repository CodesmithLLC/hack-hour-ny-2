...
function getPINs(observed) {
  if (observed.length === 1) return observed.toString();
  const pad = [
    ['1','2','3'],
    ['4','5','6'],
    ['7','8','9'],
  ...
  const observedStr = observed.toString();
  const arr = [];
  let num = observedStr[0];
  let row = 0;
  let column = 0;
  ...
  const possibleNums = [num];
  if (row - 1 >= 0) possibleNums.push(pad[row - 1][column]);
  if (row + 1 <= 3) possibleNums.push(pad[row + 1][column]);
  if (column - 1 >= 0) possibleNums.push(pad[row][column - 1]);
  if (column + 1 <= 2) possibleNums.push(pad[row][column + 1]);

  for (let i = 0; i < possibleNums.length; i++) {
    let combo = possibleNums[i] + getPINs(observed.slice(1));
    arr.push(combo);
  }
